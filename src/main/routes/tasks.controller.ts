import { AddTasksController, DeleteTaskController, ListTasksController, UpdateTaskController } from '@/application/controllers/tasks'
import { nestResponseAdapter } from '@/main/adapters'
import { type HttpResponse } from '@/application/helpers'
import { AddTasksDto, DeleteTaskDto, UpdateTaskDto } from '@/main/routes/dto/tasks'
...
import { Body, Controller, Post, Request, Res, Put, Get, Delete } from '@nestjs/common'
...
export class TasksController {
  constructor (
    ...
    private readonly deleteTask: DeleteTaskController
  ) {}
  ...
  @ApiInternalServerErrorResponse(swaggerInternalServerError())
  @Delete('/delete-task')
  async delete (@Request() req, @Body() input: DeleteTaskDto, @Res() res): Promise<HttpResponse> {
    const response = await this.deleteTask.handle({ ...input, ...req.user })
    return await nestResponseAdapter(response, res)
  }
}
